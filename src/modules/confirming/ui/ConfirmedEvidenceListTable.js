...
import {
  Button, IconButton, Stack, Table, TableBody, TableCell,
  TableContainer, TableHead, TablePagination, TableRow
} from '@mui/material';
import { Delete } from '@mui/icons-material';
import { FileUpload } from '@mui/icons-material';
import { getImageData } from '../../../react/Actions/electionActions';
import { uploadToGw } from '../../../react/Actions/gwActions';
import { ConfirmedColumnDefinitions } from '../../../react/Components/ColumnDefinitions';
...
function ConfirmedEvidenceListTable(props) {
  const {
    data, declareProperties, user, onGwUploaded, OnDeleteEvidence,
  } = props;
  ...
  const handleUpload = async () => {
    ...
    const uploadResult = await uploadToGw(uploadData, user.taxId, user.token);
    onGwUploaded(declareProperties.clientTaxId, uploadResult);
  };

  const handleDelete = async (ticket) => {
    await OnDeleteEvidence(declareProperties.clientTaxId, '04', ticket);
  };
  ...
  return (
    <>
          ...
          <TableBody>
            {rowData.map((row) => (
              ...
              >
                ...
                <TableCell handleDelete={handleDelete}>
      ...
      <TablePagination
        ...
        component="div"
        count={ConfirmedColumnDefinitions.length}
