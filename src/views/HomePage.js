...
function HomePage() {
    ...
    const handleSendImageToIdentify = async (event, data) => {
        ...
        const sentIdentifyResult = await recognizeAsync(appState.auth.user.token, ownerId, sendToIdentifyData)
    ...
    const handleGetIdentifyResult = async (event, data) => {
        ...
        for (let i = 0; i < keyList.length; i++) {
            ...
            const identifyResult = await getIdentifyResult(appState.auth.user.token, ownerId, {
            ...
            <FormControl sx={{width: '25%'}}>
                ...
                >
                    ...
                    {appState.appData.clientLists.map((client) => (
                        <MenuItem key={client.taxId.id}
                                  value={client.taxId.id}>{client.taxId.id}{' '}{client.businessName}</MenuItem>
                    ))}
                </TextField>
            </FormControl>
            <FormControl sx={{width: '25%'}}>
