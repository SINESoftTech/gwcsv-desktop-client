...
const validEvidenceType = {
  ...
  A8001: (json, assignMap) => {
    ...
    const { declarationId } = json;
    const isDeclarationIdOk = declarationId.length === 14 ? '' : 'declarationId';
    return [isEvidenceNumberOk, isDeclarationIdOk];
  },
  '': (json, assignMap) => ['gwEvidenceType', 'evidenceType', 'evidenceNumber'],
  undefined: (json, assignMap) => ['gwEvidenceType', 'evidenceType', 'evidenceNumber'],
};
...
const validTaxMoney = (json) => {
  ...
  const validResult = validTaxType[json.taxType](json).concat(validTax(json));
