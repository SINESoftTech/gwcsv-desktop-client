...
export async function loginUser(dispatch, loginPayload) {
  ...
  try {
    ...
    const response = await axios.post('/auth/login', JSON.stringify(loginPayload)); // await fetch(`${ROOT_URL}/auth/login`, requestOptions);
    const { data } = response;
    const user = { taxId: loginPayload.taxId, username: loginPayload.username };
    if (data.token) {
...
const uploadToGwStrategy = {
  async TRIPLE_GUI(payload, imageBlob, accountingFirmTaxId, token) {
    return await uploadGUI(payload, imageBlob, accountingFirmTaxId, token);
  },
  async DUPLICATE_CASH_REGISTER_GUI(payload, imageBlob, accountingFirmTaxId, token) {
    return await uploadGUI(payload, imageBlob, accountingFirmTaxId, token);
  },
  async TRIPLE_CASH_REGISTER_GUI(payload, imageBlob, accountingFirmTaxId, token) {
    return await uploadGUI(payload, imageBlob, accountingFirmTaxId, token);
  },
  async EGUI(payload, imageBlob, accountingFirmTaxId, token) {
    return await uploadGUI(payload, imageBlob, accountingFirmTaxId, token);
  },
  async ELECTRIC_BILL(payload, imageBlob, accountingFirmTaxId, token) {
    return await uploadBill(payload, imageBlob, accountingFirmTaxId, token);
  },
  async WATER_BILL(payload, imageBlob, accountingFirmTaxId, token) {
    return await uploadBill(payload, imageBlob, accountingFirmTaxId, token);
  },
  async TELECOM_BILL(payload, imageBlob, accountingFirmTaxId, token) {
    return await uploadBill(payload, imageBlob, accountingFirmTaxId, token);
  },
  async CUSTOMS_TAXABLE_EVIDENCE(payload, imageBlob, accountingFirmTaxId, token) {
...
export const getAllClientList = async (dispatch, username, taxId, token) => {
  const requestOptions = {
    headers: { 'Content-Type': 'application/json', Authorization: token, taxId },
  };
